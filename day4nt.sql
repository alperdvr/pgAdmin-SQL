...
SELECT COUNT(id) FROM employees3; --7, null varsa sayilmaz
...
SELECT calisan_isim isim FROM workers; --AS kullanmak sart degildir

SELECT calisan_isim isim FROM workers w; --tablo ismi w oldu
...
SELECT *
FROM employees3
WHERE maas = 7000; --calisir ama olmaz cunku 7000'i biz yazdik

SELECT *
FROM employees3
WHERE maas = (SELECT MAX(maas) FROM employees3); 
...
FROM march AS m --march tablosunun adi m oldu
